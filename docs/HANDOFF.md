...
| The `.pfr` parser refuses hostile input | **Verified** for the 32 cases in `tests/fuzz`. 14 of the first 22 were accepted by the previous parser; 5 cover the `state:` line. The security audit (2026-09-25) added 5, and its 3 hostile ones were accepted before its fixes: cycle stamps with `end_cycles: 0` (a run to the verifier's timeout, finding 11), and a header line over the 1024-byte buffer, which `fgets` used to split into two lines (finding 7). 300,005 more mutation cases after the fixes, no finding |
| `ram[]` has no overflow at its end | **Fixed, 2026-09-25** (security audit, finding 3): a 16- or 32-bit access at 0xFFFFFD-0xFFFFFF wrote up to 3 bytes past the buffer. `RAM_ALLOC` gives it 4 spare bytes. The game never goes there; the golden gate at `bfe1dc9` (WSL, gcc, `GOG/`, 2026-09-25) matched all five vectors on every check, so no run changed |
| `make HARDEN=1` | **Built** under WSL (gcc 13): stack canary, `_FORTIFY_SOURCE=2` (the `_chk` functions are linked), full RELRO and BIND_NOW. The verifier's image uses it (pfemu-service's Dockerfile). No arithmetic changes, so a run replays the same; **not yet shown** by the golden gate for a `HARDEN=1` build (the plain build at `bfe1dc9` passed it) |
