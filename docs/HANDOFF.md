...
| `ram[]` has no overflow at its end | **Fixed, 2026-09-25** (security audit, finding 3): a 16- or 32-bit access at 0xFFFFFD-0xFFFFFF wrote up to 3 bytes past the buffer. `RAM_ALLOC` gives it 4 spare bytes. The game never goes there; the golden gate is how that is shown for a build, not yet run on it |
| `make HARDEN=1` | **Built** under WSL (gcc 13): stack canary, `_FORTIFY_SOURCE=2` (the `_chk` functions are linked), full RELRO and BIND_NOW. The verifier's image uses it (pfemu-service's Dockerfile). No arithmetic changes, so a run replays the same; **not yet shown** by the golden gate |
