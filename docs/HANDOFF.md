...
| The verdict matches the report | **Verified** on both hosts (laptop WSL/gcc and the Debian Mac Mini), on the vector with a score. The JSON `best` and the `[RANKABLE]` line printed by the same replay both say 20,652,570, and the status is `verified`. This is the half the stub tests cannot see: the accessors feed the emitter the right numbers in a real run. |
| The golden vector pins **rankability** | **Verified**, and passing on the Mac Mini with the full eight-token comparison. `deluxe-table1-partyon-295s.expected` was regenerated and now ends `attract yes`. The diff was that one token plus the column comment. Header, hashes and all 15 frames came out unchanged, which is a good check on its own. Until now the file had seven tokens, and `run.sh` quietly cut its own side down to match |
