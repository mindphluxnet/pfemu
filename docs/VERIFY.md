...
**Status: not implemented.** This is a plan, not a description of the code.
Nothing here exists yet. Two spikes decide whether any of it gets built
...
**The ball counter is already located.** `fantasies_patch_balls()`
(fantasies.c:905) finds the ball-lost handler by masked opcode signature and
stashes `balls_counter_val[tn]`, the DS offset of `balls_left`. It does not
currently resolve a segment for that offset, because it patches the
instruction rather than reading the counter - but the histogram `data_seg`
discovery used by the spring, tilt and jump-velocity locators is right there,
`best < 8` refusal path included. Reading `balls_left` live is a small,
fully-precedented addition.
...
- **DRAINED** - the ball-lost handler runs with `balls_left` hitting 0.
  **Provisional only. This is not the end of the attempt.**
- **ENDED** - the table returns to attract mode. This is the authoritative
  terminal signal and the only point at which the score may be sampled.

### Why `balls_left` reaching 0 does not end an attempt
...
- **Extra balls awarded during play.** `balls_left` is not monotonic. The
  existing signature comment in `fantasies_patch_balls()` already shows the
  ball-lost handler containing an increment path that the match jumps over
  (`74 0A ...`) - so the counter can go up as well as down, mid-game.

So the segmenter must treat DRAINED as "possibly over" and wait for attract
mode to commit. Find the attract-mode entry in `FANTASIE.ASM`, signature-scan
for it the same way as everything else in `fantasies.c`, and hook it through
the existing `cpu_step()` address-hook mechanism.
...
- Where does the game store the **player count**, and is it readable at the
...
- Do **Angle** and **Scrolling** affect physics, or only the view? Pinned
  ...
  like Resolution.
- Does the **score** survive as a single readable variable across all three
  ranked releases, or does each need its own signature?
- Is `speed` truly guest-invisible during replay?
- Where is **attract-mode entry** in `FANTASIE.ASM`, and does it have a
  signature unique across the three ranked releases? The whole ENDED
  transition rests on this.
- Does the **match** simply restore `balls_left`, or does it re-enter play by
  some other route? Whichever it is, the score must keep accumulating into
  the same attempt rather than starting a new one.
- How does the ball-lost handler behave under **multiball**? The counter
  tracks balls remaining rather than balls in play, so it should be fine -
  but confirm, since the same handler carries the extra-ball increment path.
...
- **Spike A - score and segmentation.** Windows, no port, no server: a
  `-scoredbg` flag printing the per-attempt table the verifier would emit.
  Validate by playing normally and comparing against the DMD. This is the
  larger unknown and it is pure game archaeology; if the score cannot be
  located reliably across the three ranked releases, the project is over for
  a few evenings' work instead of after a port.
